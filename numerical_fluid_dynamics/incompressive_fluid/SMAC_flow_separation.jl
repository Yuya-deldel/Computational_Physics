...
mutable struct SMAC 
    ...
    function SMAC(X_grid, Y_grid, cell_reynolds; size_X=5.0, size_Y=1.0, velocity_of_inflow=2.0, sor_acceleration_rate=1.8, sor_convergence_criterion=0.000001, max_iter=4000, cfl=0.2)
        dx = size_X / X_grid
        dy = size_Y / Y_grid
        t = 0.0 
        viscosity_coef = 1.0 / cell_reynolds
        dt = cfl / (velocity_of_inflow / dx + 2.0 * viscosity_coef * (1.0 / dx ^ 2 + 1.0 / dy ^ 2))
        step_counter = 0
        dv_max = 0.0
        ...
        new(X_grid, Y_grid, size_X, size_Y, dx, dy, t, dt, step_counter, dv_max, viscosity_coef, 
            ...
            _ae, _aw, _an, _as, _ap, _b, velocity_of_inflow)
    end
end
...
function boundary_condition!(smac)
    ...
    for j = 2 : smac.Y_grid + 1 
        ...
        smac.pressure[smac.X_grid + 2, j] = -smac.pressure[smac.X_grid + 1, j]
    end
end
...
function _sor_pressure_correction!(smac)
    ...
    # boundary condition
    for j = 2 : smac.Y_grid + 1
        smac.pressure_correction[smac.X_grid + 2, j] = smac.pressure_correction[smac.X_grid + 1, j]
    end
end

function pressure_boundary!(smac)
    for i = 2 : smac.X_grid + 1
        smac.pressure[i, 1] = smac.pressure[i, 2]
        smac.pressure[i, smac.Y_grid + 2] = smac.pressure[i, smac.Y_grid + 1]
    end
    for j = 1 : smac.Y_grid + 2
...
function plot_velocity(smac; decimation_x=1, decimation_y=1, scale=1.0)
    ...
    for i = 2 : decimation_x : smac.X_grid + 1
        for j = 2 : decimation_y : smac.Y_grid + 1
