...
do{
  ...
  for(j=1; j<=nj; j++){
    ...
    b[i][j] = (p[i][j]-p[i+1][j])*dy + (1-alpha_u)/alpha_u*ap[i][j]*uo[i][j];
  ...
  for(j=1; j<nj; j++){
    ...
    b[i][j] = (p[i][j]-p[i][j+1])*dx + (1-alpha_u)/alpha_u*ap[i][j]*vo[i][j];
    ap[i][j] = ap[i][j]/alpha_u;
  ...
  for(i=1; i<=ni; i++){
  for(j=1; j<=nj; j++){
    if(i != 1)  aw[i][j] = dy*dy/aup[i-1][j]; else aw[i][j] = 0;
    if(i != ni) ae[i][j] = dy*dy/aup[i][j];   else ae[i][j] = 0;
    if(j != 1)  as[i][j] = dx*dx/avp[i][j-1]; else as[i][j] = 0;
    if(j != nj) an[i][j] = dx*dx/avp[i][j];   else an[i][j] = 0;
    ap[i][j] = aw[i][j] + ae[i][j] + as[i][j] + an[i][j];
    b[i][j] = -( (u[i][j]-u[i-1][j])*dy + (v[i][j]-v[i][j-1])*dx );
  }}
  ...
  for(i=1; i<=ni; i++){
  for(j=1; j<=nj; j++){
    if(i != ni) u[i][j] = u[i][j] + (pc[i][j]-pc[i+1][j])*dy/aup[i][j];
    if(j != nj) v[i][j] = v[i][j] + (pc[i][j]-pc[i][j+1])*dx/avp[i][j];
...
void coeff(){
  //** 一次風上法 **
//  aw[i][j] = (MAX(uw,0) + visc/dx)*dy;
//  ae[i][j] = (MAX(-ue,0) + visc/dx)*dy;
//  as[i][j] = (MAX(vs,0) + visc/dy)*dx;
//  an[i][j] = (MAX(-vn,0) + visc/dy)*dx;

 //** 中心法 **
  aw[i][j] = (uw/2 + visc/dx)*dy;
  ae[i][j] = (-ue/2 + visc/dx)*dy;
  as[i][j] = (vs/2 + visc/dy)*dx;
  an[i][j] = (-vn/2 + visc/dy)*dx;
