/************************************************
 cavity_frac_colo.cpp
 正方形キャビティ流れ（コロケート格子，フラクショナルステップ）
************************************************/
...
void sor_p();   // 圧力のポアソン式をSOR法で解くルーチン
FILE *fp ;      // gnuplotとの接続のためのファイルポインタ
void PlotVector(), PlotStreamline(), PlotPresure(), SaveData(); // 結果の表示ルーチン
...
double cn= 0.5,             // 時間きざみを決定する係数 (0<cn<1)
       ...
       time, dt,            // 時刻、時間きざみ
       dudt, dvdt, vold,
       ue[ni+2][nj+2] , vn[ni+2][nj+2]   // セル界面の速度（右界面，上界面）
       ;
int n,                      // 時刻ステップ
    nend= 5000;              // 計算ステップ数
    //nend= 10000;              // 計算ステップ数
...
do {
  ...
  for(i=1; i<=ni; i++){
    ...
    v[i][nj+1] = -v[i][nj];     // 上境界 v=0
  }
  for(j=1; j<=nj; j++){
    u[0][j] = -u[1][j];         // 左境界 u=0
    v[0][j] = -v[1][j];         // 左境界 v=0
    u[ni+1][j] = -u[ni][j];     // 右境界 u=0
  ...
  for(i=0; i<=ni; i++){
  for(j=0; j<=nj; j++){
//  flux_e[i][j] = ue[i][j]*(u[i+1][j]+u[i][j])/2 - (visc/dx + fabs(ue[i][j])/2)*(u[i+1][j]-u[i][j]); //１次風上
//  flux_n[i][j] = vn[i][j]*(u[i][j+1]+u[i][j])/2 - (visc/dy + fabs(vn[i][j])/2)*(u[i][j+1]-u[i][j]); //１次風上

    flux_e[i][j] = ue[i][j]*(u[i+1][j]+u[i][j])/2 - visc/dx*(u[i+1][j]-u[i][j]);              //中心
    flux_n[i][j] = vn[i][j]*(u[i][j+1]+u[i][j])/2 - visc/dy*(u[i][j+1]-u[i][j]);              //中心
  }}
  ...
  for(i=0; i<=ni; i++){
  for(j=0; j<=nj; j++){
//  flux_e[i][j] = ue[i][j]*(v[i+1][j]+v[i][j])/2 - (visc/dx + fabs(ue[i][j])/2)*(v[i+1][j]-v[i][j]); //１次風上
//  flux_n[i][j] = vn[i][j]*(v[i][j+1]+v[i][j])/2 - (visc/dy + fabs(vn[i][j])/2)*(v[i][j+1]-v[i][j]); //１次風上

    flux_e[i][j] = ue[i][j]*(v[i+1][j]+v[i][j])/2 - visc/dx*(v[i+1][j]-v[i][j]);              //中心
    flux_n[i][j] = vn[i][j]*(v[i][j+1]+v[i][j])/2 - visc/dy*(v[i][j+1]-v[i][j]);              //中心
  }}
  ...
  for(i=1; i<=ni; i++){
  for(j=1; j<=nj; j++){
    if(i != ni) ue[i][j] = (unew[i][j] + unew[i+1][j])/2;
    if(j != nj) vn[i][j] = (vnew[i][j] + vnew[i][j+1])/2;
  }}

  // divergence
  for(i=1; i<=ni; i++){
  for(j=1; j<=nj; j++){
    b[i][j] = -( (ue[i][j]-ue[i-1][j])/dx + (vn[i][j]-vn[i][j-1])/dy )/dt;
  }}
  ...
  for(j=1; j<=nj; j++){
    vold = v[i][j];
    u[i][j] = unew[i][j] + (p[i-1][j]-p[i+1][j])/2/dx*dt;
    v[i][j] = vnew[i][j] + (p[i][j-1]-p[i][j+1])/2/dy*dt;
    dvmax = MAX(dvmax, fabs(v[i][j]-vold));

    if(i != ni) ue[i][j] = ue[i][j] + (p[i][j]-p[i+1][j])/dx*dt;
    if(j != nj) vn[i][j] = vn[i][j] + (p[i][j]-p[i][j+1])/dy*dt;
  }}
...
void sor_p(){
  ...
  do {
    ...
    for(i=1; i<=ni; i++) {
    for(j=1; j<=nj; j++){
      pnew = (aw[i][j]*p[i-1][j]+ae[i][j]*p[i+1][j]
              +as[i][j]*p[i][j-1]+an[i][j]*p[i][j+1]+b[i][j])/ap[i][j] ;
      err = MAX(err, fabs(pnew-p[i][j]));
      p[i][j] = p[i][j] + alpha*(pnew-p[i][j]);
    }}
