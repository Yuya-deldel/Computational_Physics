//一次元オイラー方程式の一次精度有限体積法（方法２）
//空間再構築：Lax-Wendroff + Piecewise constant + Flux limiter
//時間積分：一段オイラー前進法
#include <stdio.h>
#include <math.h>
#include <stdlib.h>
...
double f1_high[i_max + 1];  // 密度流束 (Lax-Wendroff)
double f2_high[i_max + 1];  // 運動量流束 (Lax-Wendroff)
double f3_high[i_max + 1];  // 全エネルギー流束 (Lax-Wendroff)

double f1_low[i_max + 1];   // 密度流束 (1st-order)
double f2_low[i_max + 1];   // 運動量流束 (1st-order)
double f3_low[i_max + 1];   // 全エネルギー流束 (1st-order)
...
void first_order(int);                // 一次精度(低精度数値流束)
void cal_flux(int);                   // 数値流束を計算する関数
double sign(double);                  // 変数の符号を求める関数
double minmod1(double,double);         // minmodリミター
double limiter(double, double);       // 対称minmodリミター
...
void graph();                         // 結果可視化関数
void graph1();                         // 結果の可視化グラフをファイルに保存関数
...
{	
	...
	for (i = 0; i <= i_max - 1; i++) {
		...
		rhou_star[i + 1] = 0.5*(rhou[i + 1] + rhou[i]) - 0.5*dt*((rhou[i + 1] * rhou[i + 1] / rho[i + 1] + p1) - (rhou[i] * rhou[i] / rho[i] + p2)) / dx;
		rhoE_star[i + 1] = 0.5*(rhoE[i + 1] + rhoE[i]) - 0.5*dt*((rhou[i + 1] * ((rhoE[i + 1] + p1) / rho[i + 1])) - (rhou[i] * ((rhoE[i] + p2) / rho[i]))) / dx;
	}
	for (i = 0; i <= i_max - 1; i++) {
		f1_high[i] = rhou_star[i];
		p = (gamma - 1.0)*(rhoE_star[i]-0.5*rhou_star[i]*rhou_star[i]/rho_star[i]);
		f2_high[i] = rhou_star[i]*rhou_star[i]/rho_star[i] + p;
		f3_high[i] = rhou_star[i]*(rhoE_star[i]+p)/rho_star[i];
	}
}

void reconstruction_pc() // ０次補間による空間再構築
{
	for (i = 1; i <= i_max - 1; i++) {
		/* >>>>　セル境界（i-1/2）左側の値：一定補間（０次精度） >>>> */
		rho_L[i]  = rho[i - 1];
		rhou_L[i] = rhou[i - 1];
		rhoE_L[i] = rhoE[i - 1];
		/* >>>>　セル境界（i-1/2）右側の値：一定補間（０次精度） >>>> */
		rho_R[i]  = rho[i];
		rhou_R[i] = rhou[i];
		rhoE_R[i] = rhoE[i];
	}
...
void first_order(int riemann_option)
{
	reconstruction_pc();
...
void cal_flux(int sw1)
{
	double omega_L,omega_R,phi;
	for (i = 1; i <= i_max - 2; i++) {
		omega_L = (rho[i]-rho[i-1])                         // セル境界(i+1/2)の左側の勾配比
			/ (rho[i+1] - rho[i] + sign(rho[i+1] - rho[i])*1.0e-5);
		omega_R = (rho[i+2] - rho[i+1])                   // セル境界(i+1/2)の右側の勾配比
			/ (rho[i+1] - rho[i] + sign(rho[i+1] - rho[i])*1.0e-5);
		switch (sw1)
		{                                          // リミターを計算する
		case 1:                                    // minmod
			phi = minmod1(omega_L,omega_R);
			break;
		case 2:                                    // max
			phi = limiter(omega_L, omega_R);
			break;
		default:
			printf("空間再構築法を正しく選択されていません。");
			break;
		}
		f1[i+1] = f1_low[i+1] + phi * (f1_high[i + 1] - f1_low[i + 1]);
	}
	for (i = 1; i <= i_max - 2; i++) {
		omega_L = (rhou[i]-rhou[i-1])                         // セル境界(i+1/2)の左側の勾配比
			/ (rhou[i+1] - rhou[i] + sign(rhou[i+1] - rhou[i])*1.0e-5);
		omega_R = (rhou[i+2] - rhou[i+1])                   // セル境界(i+1/2)の右側の勾配比
			/ (rhou[i+1] - rhou[i] + sign(rhou[i+1] - rhou[i])*1.0e-5);
		switch (sw1)
		{                                          // リミターを計算する
		case 1:                                    // minmod
		...
		f2[i + 1] = f2_low[i + 1] + phi * (f2_high[i + 1] - f2_low[i + 1]);
	}
	for (i = 1; i <= i_max - 2; i++) {
		omega_L = (rhoE[i]-rhoE[i-1])                         // セル境界(i+1/2)の左側の勾配比
			/ (rhoE[i+1] - rhoE[i] + sign(rhoE[i+1] - rhoE[i])*1.0e-5);
		omega_R = (rhoE[i+2] - rhoE[i+1])                   // セル境界(i+1/2)の右側の勾配比
			/ (rhoE[i+1] - rhoE[i] + sign(rhoE[i+1] - rhoE[i])*1.0e-5);
		switch (sw1)
		{                                          // リミターを計算する
		case 1:                                    // minmod
		...
		f3[i + 1] = f3_low[i + 1] + phi * (f3_high[i + 1] - f3_low[i + 1]);
	}
	/* >>>> 左端における数値流束の境界条件 >>>> */
	f1[1] = f1[2];
	f2[1] = f2[2];
...
double minmod1(double x,double y){                           // Minmod limiter
	double a;
	a = x*y;
	if (x >=0.0&&y>=0.0) return fmin(1.0,fmin(x,y));
	else return 0.0;
}


double minmod(double x, double y) {                           // Minmod limiter
	double a;
	a = fmin(x, y);
	return fmax(0.0, fmin(1.0, a));
}
...
double limiter(double x, double y)
{
	double a;
	a = fmin(2*y,0.5*(x+y));
	return fmax(0.0, fmin(2.0,fmin(2.2*x, a)));
}
...
// Stagger-Warming のリーマンソルバー（flux vector splitting 法）
{
	...
	/* >>>> 右端における数値流束の境界条件 >>>> */
	f1_low[i_max] = f1_low[i_max - 1];
	f2_low[i_max] = f2_low[i_max - 1];
	f3_low[i_max] = f3_low[i_max - 1];
