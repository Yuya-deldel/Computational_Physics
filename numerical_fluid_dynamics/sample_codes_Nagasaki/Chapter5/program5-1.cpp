//一次元オイラー方程式の一次精度有限体積法
//リーマンソルバー: ①　flux vector splitting）；②　flux difference splitting
//時間積分：一段オイラー前進法
#include <stdio.h>
#include <math.h>
#include <stdlib.h>
...
// Stagger-Warming のリーマンソルバー（flux vector splitting 法）
{
	...
	double lambda1_p, lambda2_p, lambda3_p, lambda1_n, lambda2_n, 
		   lambda3_n;
...
void update()                                      // 時間積分（一次オイラー前進法）
{
	for (i = 1; i <= i_max - 1; i++) {
		Lh_rho[i]  = (f1[i] - f1[i + 1])/ (x[i + 1] - x[i]);
		Lh_rhou[i] = (f2[i] - f2[i + 1])/ (x[i + 1] - x[i]);
		Lh_rhoE[i] = (f3[i] - f3[i + 1])/ (x[i + 1] - x[i]);
		rho[i]  = rho[i]  + dt * Lh_rho[i];
		rhou[i] = rhou[i] + dt * Lh_rhou[i];
		rhoE[i] = rhoE[i] + dt * Lh_rhoE[i];
	}
}
