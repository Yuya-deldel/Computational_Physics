/* 一次元バーガース方程式の一次精度有限体積法 (Harten ソルバー) */
...
void riemann_harten();//リーマン・ソルバー関数（Hartenスキーム）
...
void exact(int sw1) {
	...
	case 1:		//　初期の変数値（滑らかな分布）
	for (i = 0; i <= i_max + 1; i++) {
		c = 2.*M_PI* (i_max + 3) / i_max;
		f=ue[i] - 0.5*(1.1 + sin(c*(x[i] - ue[i] * t) ));
		df = 1.0 + 0.5*c * cos(c*(x[i] - ue[i] * t))*t;
		while (fabs(f) >= 1.0e-4) {  
