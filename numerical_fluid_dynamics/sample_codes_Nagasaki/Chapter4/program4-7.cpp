/* 一次元線形移流方程式の二次精度TVD有限体積法 (Flux-limiter) */
#define _USE_MATH_DEFINES
#include <stdio.h>
#include <math.h>
#include <stdlib.h>

#define i_max 100           //格子セル数
#define XL -1.0            //計算領域左端の座標
#define XR 1.0            //計算領域右端の座標
#define a 1.0             //線形移流方程式の移流速度
#define tstop 2.0            //計算停止時刻
FILE *fp;				 // 描画ソフトgnuplotの実行ファイルのポインタ

int i;    //セル番号； (i番目セルのセル左境界の番号はi、右辺境界の番号はi+1とする)
double x[i_max +1];    //セル境界の座標
double u[i_max];       //セル平均値
double u_star[i_max];       //セル境界中間値
double ue[i_max];       //セル平均値
double ul[i_max+1];    //セル境界左側の変数値
double ur[i_max+1];    //セル境界右側の変数値
double f[i_max+1];    //セル境界の流束
double f_high[i_max + 1], f_low[i_max + 1];    //高精度流束と低精度流束
double dx;            //格子間隔（等間隔とする）
double dt;            //時間刻み
int n;            //時間ステップ
double t;            //計算時間

/*
　    計算変数と数値流速の配置
 
     セル境界　　　　　 セル境界
  -------|-----------------|-------
       f[i]     u[i]    f[i+1]　　　==> 計算コードの変数定義

	  f_i-1/2   u_i     f_i+1/2     ==> テキストの標記


　  　境界条件
    u[0]     u[1]                    u[i_max-1]  u[i_max]
 ---------|-------|  ---- ---  ----|-----------|---------
         [1]     [2]           [i_max-2]    [i_max-1]
		      計算境界		    計算境界

*/

double sign(double);
double minmod(double);
double minmod1(double, double);
void graph(); //gnuplotを用いて描画する関数
void graph1(); //gnuplotを用いて描画する関数（output.epsへ出力）
void initc(int); //初期条件を設定する関数
void roe_1st(); // Roe流束（一次精度）
void lax_wendroff(); // Lax - Wendroff流束（二次精度）
void flux(); //数値流束（TVD流束リミター）
void update();//時間前進を計算する関数
void bc();//境界条件を設定する関数（周期境界条件）
void exact(int);//厳密解を計算する関数

int main() //　main　関数
{
	int sw1;
	fp = _popen("C:/gnuplot/bin/gnuplot", "w"); //描画ツールgnuplotのファイルポインタ指定
	fprintf(fp, "set terminal windows title 'Solution plots' size 800,600 \n");//可視化の出力先指定（PCの画面に表示）

	printf("線形移流方程式の初期値を選ぶ\n");
	printf("滑らかな分布（正弦波）：１　\n");
	printf("不連続な分布（矩形波）：２　\n");
	scanf_s("%d", &sw1);

	n = 0;  
	t = 0.0;
	initc(sw1);	// 計算格子，時間刻み，初期条件を設定する
	exact(sw1);
	graph();	// 計算結果をグラフで表示する
	getchar();

	while (t <= tstop) //　メインループ
	{
		n = n + 1;
		t = t + dt;
		roe_1st();	// Roe流束（一次精度）
		lax_wendroff(); //Lax - Wendroff流束（二次精度）
		flux(); //数値流束
		update();	// 時間積分
		bc();	// 境界条件
		exact(sw1);// 厳密解
		graph();	// 計算結果をグラフで表示する
		printf("n=%d, t=%f, \n", n, t);
	}
    //*****************************************
    graph1();	// 計算結果のグラフを保存する
    //*****************************************
	printf("Press Enter key to finish.\n");
    getchar();
}


void initc(int sw1) { // 計算格子，時間刻み，初期条件を設定する
	dx = (XR - XL) / (double)i_max;  //　格子間隔
	dt = 0.2*dx; //時間刻み
	x[0] = XL;
	for (i = 1; i <= i_max + 1; i++) {
		x[i] = x[i - 1] + dx;  //　格子点の座標
	}

	for (i = 0; i <= i_max; i++) {
		u[i] = 0.0;  //　変数値を０に初期化
	}

	switch (sw1)
	{
	case 1:		//　初期の変数値（滑らかな分布）
		for (i = 0; i <= i_max + 1; i++) {
			u[i] = 0.5*(1.1 + sin(2.*M_PI*(x[i] - a * t) * (i_max + 3) / (i_max)));
		}
		break;
	case 2:		//　初期の変数値（不連続な分布）
		for (i = 0; i <= i_max; i++) {
			u[i] = 0.1;  
		}
		for (i = i_max / 2 - 10; i <= i_max / 2 + 10; i++) {
			u[i] = 1.0;
		}
		break;
	default:
		printf("初期値を正しく選択されていません。");
		break;
	}
}

void roe_1st() {
	//	一次精度流束を計算する
	for (i = 2; i <= i_max - 1; i++) {
		f_low[i] = 1.0 / 2.0*(a*u[i-1] + a * u[i]) - 1.0 / 2.0*fabs(a)*(u[i] - u[i-1]);
	}
}

void lax_wendroff() {
	//	二次精度流束を計算する
	for (i = 2; i <= i_max - 1; i++) {
		u_star[i] = 0.5*(u[i] + u[i-1]) - 0.5*dt*(u[i] - u[i-1]) / dx;
		f_high[i] = a*u_star[i];
	}
}

void flux() {
	//	TVDリミターにより流束を計算する
	double rL, rR, phiL, phiR,phi;
//	double alpha_12, deltaL, deltaR;
	for (i = 1; i <= i_max - 1; i++) {
		rL = (u[i] - u[i - 1]) //セル境界(i+1/2)の左側の勾配比
			/ (u[i + 1] - u[i] + sign(u[i + 1] - u[i])*1.0e-5);
		rR = (u[i + 2] - u[i + 1]) //セル境界(i+1/2)の右側の勾配比
			/ (u[i + 1] - u[i] + sign(u[i + 1] - u[i])*1.0e-5);
		phiL = minmod(rL);
		phiR = minmod(rR);
		phi = minmod1(rL, rR);
		f[i+1] = f_low[i+1]+ phi*(f_high[i+1]- f_low[i+1]);
	}

}


void update() {
	for (i = 2; i <= i_max - 2; i++) {
		u[i] = u[i] - dt / dx * (f[i + 1] - f[i]);//計算変数を更新する
	}
}

void exact(int sw1) {
	double xc, xl, xr;
	switch (sw1)
	{
	case 1:		//　初期の変数値（滑らかな分布）
		for (i = 0; i <= i_max + 1; i++) {
			ue[i] = 0.5*(1.1 + sin(2.*M_PI*(x[i] - a * t) * (i_max + 3) / (i_max)));
		}
		break;
	case 2:		//　初期の変数値（不連続な分布）
		xc = a * t;
		xl = xc - dx * 10.;
		if (xl>1.0 - dx * 2.0)
		{
			xl = -2.0 + xl + dx * 2.0;
		}
		xr = xc + dx * 10.;
		if (xr>1.0 - dx * 2.0)
		{
			xr = -2.0 + xr + dx * 3.0;
		}
		if (xl <= xr)
		{
			for (i = 0; i <= i_max; i++) {
				ue[i] = 0.1;
				if (dx*double(i - i_max / 2) >= xl && dx*double(i - i_max / 2) <= xr)
				{
					ue[i] = 1.0;
				}
			}
		}
		if (xl >= xr)
		{
			for (i = 0; i <= i_max; i++) {
				ue[i] = 1.0;
				if (dx*double(i - i_max / 2) >= xr && dx*double(i - i_max / 2) <= xl)
				{
					ue[i] = 0.1;
				}
			}
		}
		break;
	}
}

void bc() {
	u[0] = u[i_max - 3];  //　計算領域左端の境界条件
	u[1] = u[i_max - 2];  //　計算領域左端の境界条件
	u[i_max - 1] = u[2];    //　計算領域右端の境界条件
	u[i_max] = u[3];      //　計算領域右端の境界条件
}


double minmod(double x) {	//	Minmod limiter
	return fmax(0.0, fmin(1.0, x));
}

double minmod1(double x, double y) {                           // Minmod limiter
	if (x >= 0.0&&y >= 0.0) return fmin(1.0, fmin(x, y));
	else return 0.0;
}


double sign(double x) { //変数の符号を求める関数
	if (x >= 0) return 1;
	else return -1;
}



void graph() //　計算結果の可視化ツール
{
	fprintf(fp, "set xrange [-1:1] \n"); ;//横軸表示範囲
	fprintf(fp, "set yrange [-0.25:1.25] \n"); //縦軸表示範囲

	fprintf(fp, "set xtics -1,0.5,1  \n"); //横軸目盛
	fprintf(fp, "set ytics 0,0.5,1  \n"); //縦軸目盛

	fprintf(fp, "plot 0 title 'X-axis'lw 3.5 lc rgb 'black',"); // x軸（黒線）
	fprintf(fp, " '-'title 'Exact sol.' w line lw 2.5  lc rgb 'red',");// 厳密解（赤線）
	fprintf(fp, " '-'title 'Numerical sol.' w point pt 7 ps 1 lc rgb 'blue'\n");// 数値解（青点）
	for (i = 1; i <= i_max - 1; i++) {
		fprintf(fp, "%f %f \n", x[i], ue[i]);
	}
	fprintf(fp, "e \n");

	for (i = 1; i <= i_max - 1; i++) {
		fprintf(fp, "%f %f \n", x[i], u[i]);
	}
	fprintf(fp, "e \n");

	fflush(fp);
}

void graph1() //　計算結果可視化出力
{
	fprintf(fp, "set terminal postscript eps \n");//可視化の出力先指定（epsファイルに出力）
	fprintf(fp, "set output 'output.eps \n");//出力ファイル名指定
	fprintf(fp, "set xrange [-1:1] \n"); ;//横軸表示範囲
	fprintf(fp, "set yrange [-0.25:1.25] \n"); //縦軸表示範囲
	fprintf(fp, "set xtics -1,0.5,1  \n"); //横軸目盛
	fprintf(fp, "set ytics 0,0.5,1  \n"); //縦軸目盛
	fprintf(fp, "plot 0 title 'X-axis'lw 3.5 lc rgb 'black',"); // x軸（黒線）
	fprintf(fp, " '-'title 'Exact sol.' w line lw 2.5  lc rgb 'red',");// 厳密解（赤線）
	fprintf(fp, " '-'title 'Numerical sol.' w point pt 7 ps 1 lc rgb 'blue'\n");// 数値解（青点）
    for (i = 1; i <= i_max - 1; i++) {
        fprintf(fp, "%f %f \n", x[i], ue[i]);
    }
    fprintf(fp, "e \n");

    for (i = 2; i <= i_max - 1; i++) {
        fprintf(fp, "%f %f \n", x[i], u[i]);
    }
    fprintf(fp, "e \n");
    fflush(fp);
}

