/* 一次元線形移流方程式の二次精度TVD法(van Albada limiter) */
...
void reconstruction_vanAlbada();//再構築関数（TVD法，van Albada limiter）
...
int main() //　main　関数
{
	...
	while (t <= tstop) //　メインループ
	{
		n = n + 1;
		t = t + dt;
		reconstruction_vanAlbada();	// 空間再構築
