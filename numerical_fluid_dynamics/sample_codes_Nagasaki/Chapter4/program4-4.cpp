...
void reconstruction_bw();//再構築関数（１次補間，Beam-Warming法）
...
void reconstruction_bw() {
    double alpha_12, deltaL, deltaR;
    for (i = 1; i <= i_max - 2; i++) {
        alpha_12 = a;
        deltaL = 0.5*(u[i] - u[i-1]);
        deltaR = 0.5*(u[i + 2] - u[i+1]);
