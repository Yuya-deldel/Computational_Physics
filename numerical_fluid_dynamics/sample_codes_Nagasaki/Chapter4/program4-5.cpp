...
void graph(); //gnuplotを用いて描画する関数（PC画面表示）
void graph1(); //gnuplotを用いて描画する関数（output.epsへ出力）
void initc(int); //初期条件を設定する関数
void reconstruction_fm();//再構築関数（１次補間，Fromm法）
...
void reconstruction_fm() {
    double alpha_12, deltaL, deltaR;
    for (i = 1; i <= i_max - 2; i++) {
        alpha_12 = a;
        deltaL = 0.25*(u[i+1] - u[i-1]);
        deltaR = 0.25*(u[i + 2] - u[i]);
